...
//    printf("%s\n", argv[2]);
    ...
    fgets(_thread_args->clientInfoPacket.username, sizeof (_thread_args->clientInfoPacket.username), stdin);

    send_packet(server_socket, &_thread_args->clientInfoPacket);
    close(server_socket);
